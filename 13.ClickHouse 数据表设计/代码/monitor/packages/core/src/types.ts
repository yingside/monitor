...
export interface MonitorOptions {
   ...
   *
   * 注意：这是 Transport 初始化之前的"预缓冲区"容量。
   * 正常情况下 init() 会立刻创建 Transport，此值不会被触达。
   */
  maxQueueSize?: number
  ...
  /**
   * 批量上报定时刷新间隔（ms），默认 5000（5秒）
   *
   * SDK 不会每采集一条数据就立刻上报，而是先缓存，每隔 flushInterval
   * 毫秒批量打包发出。这样可以大幅减少 HTTP 请求数量。
   *
   * 权衡：
   * - 值越小 → 数据越实时，但网络请求越频繁
   * - 值越大 → 节省流量，但数据延迟更高，页面关闭时丢失风险也越大
   */
   ...
   * 每批最多发送的事件数量，默认 10
   *
   * 如果 flushInterval 还没到，但队列已积累了 maxBatchSize 条事件，
   * 会立刻触发一次上报（"满了就发"）。
   *
   * 防止突发流量时队列无限膨胀（比如循环抛错导致大量 error 事件）。
   */
  maxBatchSize?: number
}
