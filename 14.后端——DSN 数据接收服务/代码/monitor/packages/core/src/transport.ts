...
export class Transport {
  ...
  private _sendFetch(): void {
    ...
    fetch(this.config.dsn, {
      ...
      // credentials: 'omit'：监控上报不需要携带 cookie / auth header
      // 如果不显式设置，keepalive 请求在部分浏览器下会被标记为 credentials: 'include'，
      // 导致服务端 CORS 响应 'Access-Control-Allow-Origin: *' 被浏览器拒绝
      credentials: 'omit',
      keepalive: true,
      // dsn-server（NestJS）的 ReportBatchDto 要求 { events: [...] } 结构
