...
export class MonitorDataService {
  ...
  /** 性能指标统计（均值摘要 + LCP 趋势 + 分位数 + 最慢页面 Top 10） */
  async getPerformanceStats(dto: BaseQueryDto) {
    ...
    const [avgRows, trendRows, quantileRows, topSlowPagesRows] = await Promise.all([
      ...
      // 关键指标分位数（P50 / P75 / P95）
      // quantileIf(level)(expr, cond) 仅统计满足条件的行，等价于先过滤再求分位数
      this.clickhouse.query<Record<string, string>>(
        `SELECT
           ...
           round(quantileIf(0.5)(lcp,       lcp > 0), 2)        AS lcp_p50,
           round(quantileIf(0.75)(lcp,      lcp > 0), 2)        AS lcp_p75,
           round(quantileIf(0.95)(lcp,      lcp > 0), 2)        AS lcp_p95,
           round(quantileIf(0.5)(ttfb,      ttfb > 0), 2)       AS ttfb_p50,
           round(quantileIf(0.75)(ttfb,     ttfb > 0), 2)       AS ttfb_p75,
           round(quantileIf(0.95)(ttfb,     ttfb > 0), 2)       AS ttfb_p95,
           round(quantileIf(0.5)(load_time, load_time > 0), 2)  AS load_p50,
           round(quantileIf(0.75)(load_time,load_time > 0), 2)  AS load_p75,
           round(quantileIf(0.95)(load_time,load_time > 0), 2)  AS load_p95
         FROM performance_logs
         WHERE app_id = {appId: String}
           AND created_at >= {start: String}
           AND created_at <= {end: String}`,
      ...
      // 最慢页面 Top 10（按平均 LCP 降序）
      this.clickhouse.query<{ page: string; avgLcp: string; avgFcp: string; count: string }>(
        `SELECT
           page,
           round(avgIf(lcp, lcp > 0), 2) AS avgLcp,
           round(avgIf(fcp, fcp > 0), 2) AS avgFcp,
    ...
    return {
      ...
      quantiles: {
        fcp:      { p50: Number(q['fcp_p50'] ?? 0),  p75: Number(q['fcp_p75'] ?? 0),  p95: Number(q['fcp_p95'] ?? 0) },
        lcp:      { p50: Number(q['lcp_p50'] ?? 0),  p75: Number(q['lcp_p75'] ?? 0),  p95: Number(q['lcp_p95'] ?? 0) },
        ttfb:     { p50: Number(q['ttfb_p50'] ?? 0), p75: Number(q['ttfb_p75'] ?? 0), p95: Number(q['ttfb_p95'] ?? 0) },
        loadTime: { p50: Number(q['load_p50'] ?? 0), p75: Number(q['load_p75'] ?? 0), p95: Number(q['load_p95'] ?? 0) },
      },
      topSlowPages: topSlowPagesRows.map((r) => ({
        page: r.page,
        avgLcp: Number(r.avgLcp),
        avgFcp: Number(r.avgFcp),
        count: Number(r.count),
      })),
    }
  }
