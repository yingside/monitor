...
> 本文档描述**第 16 章结束后**项目的完整状态，供后续章节开发时快速建立上下文，避免重复探索代码。
...
| 项目 | 说明 |
|---|---|
| **根目录** | `16.后端——监控平台 API 服务/代码/monitor/` |
| **包管理器** | pnpm 10.x |
| **构建工具** | Turborepo v2 |
| **Node.js** | v25.x |
...
│   │   ├── nest-cli.json
│   │   └── package.json
│   └── monitor-server/  平台 API 服务（✅ 第 16 章完成）
│   │   ├── src/
│   │   │   ├── main.ts
│   │   │   ├── app.module.ts
│   │   │   ├── common/
...
│   │   │   ├── user/ (module/service + user.entity)
│   │   │   ├── project/ (module/controller/service + project.entity + dto)
│   │   │   ├── monitor-data/ (module/controller/service + query.dto)
│   │   │   └── clickhouse/ (module/service — 只做 SELECT 查询)
│   │   ├── .env.example
│   │   ├── nest-cli.json
│   │   ├── package.json
│   │   └── tsconfig.json
...
│   └── postgres/        ← 第 16 章新增
│       ├── 01_create_tables.sql  users + projects 表定义
│       ├── 02_mock_data.sql      测试用户 + 测试项目
│       └── 03_query_examples.sql 验证查询
...
## 九、Monitor Server 状态（第 16 章新增）✅

### 服务概览

| 项目 | 值 |
|---|---|
| 包名 | `@monitor/monitor-server` |
| 端口 | 3003（可通过 `PORT` 环境变量覆盖）|
| 框架 | NestJS v10 + Express |
| 启动命令 | `pnpm monitor-server` 或 `pnpm --filter @monitor/monitor-server dev` |
| 数据库 | PostgreSQL（TypeORM）+ ClickHouse（查询用）|
...
- `JwtAuthGuard` 注册为全局 Guard（`APP_GUARD`）
- 所有接口默认需要 JWT（`Authorization: Bearer <token>`）
- 用 `@Public()` 装饰器标记公开接口（登录/注册）
...
| POST | `/projects` | 创建项目（appId 全局唯一）|
| GET | `/projects/:id` | 获取单个项目（校验所有权）|
| PATCH | `/projects/:id` | 更新项目（appId 不可修改）|
| DELETE | `/projects/:id` | 删除项目（返回 204）|

### Monitor Data API 端点

| 方法 | 路径 | 查询参数（公共）|
|---|---|---|
| GET | `/monitor/errors` | appId / startTime / endTime / page / pageSize / errorType |
| GET | `/monitor/errors/stats` | appId / startTime / endTime |
| GET | `/monitor/performance` | appId / startTime / endTime / page / pageSize |
| GET | `/monitor/performance/stats` | appId / startTime / endTime |
| GET | `/monitor/behaviors` | appId / startTime / endTime / page / pageSize |
| GET | `/monitor/behaviors/stats` | appId / startTime / endTime |
| GET | `/monitor/apis` | appId / startTime / endTime / page / pageSize / url / onlyFailed |
| GET | `/monitor/apis/stats` | appId / startTime / endTime |

- `startTime` / `endTime`：ISO 8601 字符串，未传默认最近 7 天
- 时间区间内部转为 ClickHouse DateTime 格式（`YYYY-MM-DD HH:MM:SS`）

### PostgreSQL 表结构（TypeORM synchronize 自动创建）

**users**：id(UUID) / email(唯一) / password(bcrypt) / name / created_at / updated_at

**projects**：id(UUID) / app_id(唯一) / name / description / platform / owner_id(FK→users) / created_at / updated_at
...
| `CLICKHOUSE_USER` | monitor | 用户名 |
| `CLICKHOUSE_PASSWORD` | 123456 | 密码 |
| `JWT_SECRET` | monitor_dev_secret_... | JWT 密钥（生产必须替换）|
...
ClickHouse 四张表（已有真实数据）     ← ✅
                    ↑ SELECT 查询
monitor-server (localhost:3003)       ← ✅ 第 16 章
  ↕ PostgreSQL（用户/项目管理）        ← ✅
  ↑ HTTP API（鉴权 + 业务）
React 前端平台（第 17-21 章）         ← 🏗️ 待实现
```
...
# 启动 Monitor Server（平台 API，端口 3002）
pnpm monitor-server

# 健康检查
curl http://localhost:3001/health

# 快速测试 monitor-server 注册接口
...
```sql
-- 写入 ClickHouse 模拟数据：在 DataGrip 中打开 db/clickhouse/02_mock_data.sql，全选执行

-- 写入 PostgreSQL 测试数据：在 DataGrip 中打开 db/postgres/02_mock_data.sql，全选执行
...
-- 验证 PostgreSQL 用户和项目
SELECT p.app_id, p.name, u.email AS owner FROM projects p JOIN users u ON p.owner_id = u.id;
```
