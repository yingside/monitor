...
/**
 * RegisterPage —— 注册页（第 18 章：接入真实 API）
 *
 * 第 17 章：UI 骨架
 * 第 18 章：引入 useRegister Hook，接入 POST /auth/register，
 *           注册成功后自动登录并跳转 /projects（无需用户再次登录）
 */
export default function RegisterPage() {
  ...
  const errorMessage = (() => {
    ...
    return axiosError.response?.data?.message ?? '注册失败，请稍后重试'
  })()
