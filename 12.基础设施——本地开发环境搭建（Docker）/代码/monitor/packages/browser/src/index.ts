...
// 第 10 章：API 请求监控插件（XHR / Fetch 自动采集）
export { createApiPlugin } from './plugins/api'
export type { ApiPluginOptions } from './plugins/api'
 ...
 * 特殊场景下（用户登出、关键业务节点后希望数据立刻发出）可手动调用。
 *
 * @example
 * ```ts
 * import { flush } from '@monitor/browser'
 *
 * // 用户登出前，确保队列中的数据都发出去
 * flush()
 * ```
